...
local update_urls = {
	['main']       = 'https://raw.githubusercontent.com/madokaez/AdminTools/main/AdminTool.lua',
	['events']     = 'https://raw.githubusercontent.com/madokaez/AdminTools/main/ATEvents.lua',
	['admcheat']   = 'https://raw.githubusercontent.com/madokaez/AdminTools/main/admcheat.luac',
	['adminstate'] = 'https://raw.githubusercontent.com/madokaez/AdminTools/main/adminstate.lua',
...
local function updateDownloadAll()
	if update_in_progress then return end
	update_in_progress = true
	local total, done = 0, 0
	for _ in pairs(update_urls) do total = total + 1 end
	for k, url in pairs(update_urls) do
		downloadUrlToFile(url, update_paths[k], function(id, status)
	...
	local waited = 0
	while done < total and waited < 60000 do
		wait(200); waited = waited + 200
	end
	update_in_progress = false
	sampAddChatMessage(tag .. 'Обновление завершено (' .. done .. '/' .. total .. '). Перезагрузка скриптов...', -1)
	reloadScripts()
end
...
local function TryLoadSubscriptAuto(key, base_rel)
    local wd = getWorkingDirectory()
    local luac_rel = base_rel .. ".luac"
    local lua_rel  = base_rel .. ".lua"
    local luac_abs = wd .. "/" .. luac_rel
    local lua_abs  = wd .. "/" .. lua_rel

    if doesFileExist(luac_abs) then
        return TryLoadSubscript(key, luac_rel)
    elseif doesFileExist(lua_abs) then
        return TryLoadSubscript(key, lua_rel)
    else
        sampAddChatMessage(tag .. "{FF5555}Не найден " .. base_rel .. ".luac / .lua", -1)
        return false, nil
    end
end
other_res, pother = TryLoadSubscriptAuto("others", "resource/admcheat")
...
function main()
	...
	lua_thread.create(function()
		...
		local wd = getWorkingDirectory()
		local chip_luac = wd .. "/resource/chip.luac"
		local chip_lua  = wd .. "/resource/chip.lua"
		local chip_path
		if doesFileExist(chip_luac) then
			chip_path = "resource/chip.luac"
		elseif doesFileExist(chip_lua) then
			chip_path = "resource/chip.lua"
		end
		if chip_path then
			local ok, mod = pcall(import, chip_path)
			if ok and mod then
				iplogger_res, iplp = true, mod
			else
				sampAddChatMessage(tag .. "Не удалось загрузить " .. chip_path, -1)
			end
		else
			sampAddChatMessage(tag .. "Файл chip.luac/.lua не найден", -1)
		end
	end)
	scan_logs_file()
			...
			if imgui.Button(u8"IP-логгер", imgui.ImVec2(btnW3, 30)) then
				...
				if not IsFullAccessAdmin(atlibs.getMyNick()) then
					...
					if iplogger_res then
						...
						sampAddChatMessage(tag .. "Скрипт отключен в (Дополнительно)", -1)
					end
				end
			end
			imgui.SameLine()
